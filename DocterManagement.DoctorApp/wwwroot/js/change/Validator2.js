...
Validator.minCharSpecial = (selector, message = "Nhập tối tiểu một ký tự đặt biệt") => {
	return {
		selector,
		test: function (value) {
			const regex = /(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,./?])/
...
Validator.minCharAlpha = (selector, min, message=`Vui lòng nhập tối thiểu ${min} chữ cái thường`) => {
	return {
		selector,
		test: function (value) {
			const regex = /(?=.*[a-z]).{1,}/
...
Validator.minCharAlphaUpcase = (selector, min, message = `Vui lòng nhập tối thiểu ${min} chữ hoa`) => {
	return {
		selector,
		test: function (value) {
			const regex = /(?=(.*[A-Z])).{1,}/
